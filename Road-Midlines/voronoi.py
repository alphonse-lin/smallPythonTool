...
dist = 7   # Buffer distance
point_dist = 10   # Distance among points when interpolating points within lines to add more points
grid_size = 500
threshold = 15   # Threshold for the length under which lines should be deleted
rdp_eps = 7   # Epsilon value for the RDP algorithm
margin_pos = 2   # Number of digits that should be tolerated when snapping points
folder_name = 'Road-Midlines'
path.append(folder_name)
# file_path = './{0}/geojson/sample.geojson'.format(folder_name)   # Input data path
# output_path = './{0}/geojson/sample_output2.geojson'.format(folder_name)   # Output data path
...
def explodeLineList(x):
    newList = []
    for i in range(len(x)):
        for j in range(1, len(x[i])):
            newList.append([x[i][j-1], x[i][j]])
    return newList


def deleteZItem(x):
    if type(x)==LineString:
        newLineString = LineString([xy[0:2] for xy in list(x.coords)])
    else:
        newLineString = []
        for a in x:
            thisLineString = LineString([xy[0:2] for xy in list(a.coords)])
            newLineString.append(thisLineString)
        newLineString = MultiLineString(newLineString)
    return newLineString


def getAllCities(file_path):
    from os import listdir
    allfiles = listdir(file_path)
    allfiles = list(set([x.split('.')[0] for x in allfiles]))
    return allfiles


def readData(file_path, city):
    from pandas import concat
    df = GeoDataFrame()
    file_name = '{0}.shp'.format(city)
    this_path = file_path + r'\{0}'.format(file_name)
    newdf = read_file(this_path)
    newdf.crs = {"init": "epsg:4326"}
    newdf = newdf.to_crs({"init": "epsg:32650"})
    df = concat([df, newdf], ignore_index=True)
    df.geometry = [deleteZItem(x) if x.has_z else x for x in df.geometry]
    return df


def multilines2list(multilineList):
    newlist = []
    for this_line in multilineList:
        if type(this_line)==MultiLineString:
            nl = [np.asarray(x) for x in this_line]
            newlist.extend(nl)
        else:
            nl = np.asarray(this_line)
            newlist.append(nl)
...
##### Program starts #####
# file_path = r'D:\UrbanXLab\dataCollections\全国_导航数据_按省份\output'
# province = '山东省'
# output_path = r'D:\UrbanXLab\dataCollections\单线_全国_导航数据_20200920\{0}2.geojson'.format(province)

file_path = r'D:\UrbanXLab\dataCollections\42城市道路划分'
allfiles = getAllCities(file_path)
# allfiles = [x for x in allfiles if x != '广州市']
allfiles = ['广州市']
for city in allfiles:
    start_time = time()
    output_path = r'D:\UrbanXLab\Road-Midlines\geojson\{0}.shp'.format(city)
    print(city)

    df = readData(file_path, city)
    df_geom = multilines2list(list(df.geometry))
    myGraph = Graph(df_geom)
    arrange_roads = myGraph.arrangeIntoWholeLines()
    simplified_former_road_list = [rdp(x, epsilon=rdp_eps) for x in arrange_roads]
    simplified_former_road_list = [LineString(x) for x in simplified_former_road_list]
    ...
    # Arrange road sections into whole lines
    finalGraph = Graph(final_roads)
    arrange_final_roads = finalGraph.arrangeIntoWholeLines()

    # Output
    final_roads = [LineString(x) for x in arrange_final_roads]
    final_roads = GeoDataFrame(geometry=final_roads, crs={"init": "epsg:32650"})
    final_roads.to_file(output_path, driver='ESRI Shapefile')
    print("Finish!")
    calculate_running_time(start_time)
